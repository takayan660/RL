...
class RoboschoolForwardWalker(SharedMemoryClientEnv):
    def __init__(self, power):
        self.power = power
        self.camera_x = 0
        self.walk_target_x = 0  # kilometer away
        self.walk_target_y = 0
        self.kick_target_x = 1
        self.kick_target_y = 0
    ...
    def calc_state(self):
        ...
        self.foot_xyz = (self.parts["right_foot"].pose().xyz(), self.parts["left_foot"].pose().xyz())
        self.body_rpy = body_pose.rpy()
        self.foot_rpy = (self.parts["right_foot"].pose().rpy(), self.parts["left_foot"].pose().rpy())
        z = self.body_xyz[2]
        ...
        self.kick_target_theta = np.arctan2( self.kick_target_y - self.foot_xyz[0][1], self.kick_target_x - self.foot_xyz[0][0] )
        self.kick_target_dist  = np.linalg.norm( [self.kick_target_y - (self.foot_xyz[0][1] - self.body_xyz[1]), self.kick_target_x - (self.foot_xyz[0][0] - self.body_xyz[0])] )
    ...
    def calc_potential(self):
        ...
        return - self.kick_target_dist / self.scene.dt
